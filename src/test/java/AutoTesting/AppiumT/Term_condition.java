...
public class Term_condition extends AppTest {
    private static final Logger logger = LogManager.getLogger(Term_condition.class);
    ...
    public void notifi() throws InterruptedException {
        ...
        try {
            ...
            logger.info("Scrolling to and clicking Term and condition");
            driver.findElement(AppiumBy.androidUIAutomator(
                    "new UiScrollable(new UiSelector().scrollable(true))" +
                    ".scrollIntoView(new UiSelector().description(\"Terms & Conditions\"))")).click();

            logger.info("Waiting for  content to load");
            wait.until(ExpectedConditions.presenceOfElementLocated(
                    AppiumBy.androidUIAutomator("new UiSelector().className(\"android.view.View\").instance(6)")));

            List<WebElement> textElements = driver.findElements(AppiumBy.androidUIAutomator("new UiSelector().className(\"android.view.View\").instance(6)"));
            List<String> expectedLines = Arrays.asList();

            for (String expected : expectedLines) {
                boolean found = textElements.stream()
                        .anyMatch(el -> el.getText().trim().contains(expected));
                Assert.assertTrue(found, "Expected text not found in Term and condition: " + expected);
            }

            logger.info("Successfully verified Term and condition  content");
            ...
            emailField.click();
            emailField.sendKeys("asdfsa@gmail.com");
            driver.hideKeyboard();
